...
const api = {
  git: {
    ...
    getConflictCandidateSettings: (repoPath: string) => ipcRenderer.invoke('conflict:candidate-settings', repoPath),
    setConflictCandidateEnabled: (repoPath: string, enabled: boolean) => ipcRenderer.invoke('conflict:candidate-settings-set', repoPath, enabled),
    getConflictCandidateRecords: (repoPath: string) => ipcRenderer.invoke('conflict:candidate-records', repoPath),
    getConflictCandidates: (repoPath: string, filePath: string, generation: string) => ipcRenderer.invoke('conflict:candidates', repoPath, filePath, generation),
    previewConflictCandidate: (repoPath: string, filePath: string, generation: string, candidateId: string) => ipcRenderer.invoke('conflict:candidate-preview', repoPath, filePath, generation, candidateId),
    recordConflictCandidate: (repoPath: string, filePath: string, generation: string, regionId: string, proposedBytes: string) => ipcRenderer.invoke('conflict:candidate-record', repoPath, filePath, generation, regionId, proposedBytes),
    forgetConflictCandidateRecords: (repoPath: string, id?: string) => ipcRenderer.invoke('conflict:candidate-forget', repoPath, id),
    getPartialDiff: (repoPath: string, filePath: string, target: 'stage' | 'unstage' | 'discard' | 'staged-discard') => ipcRenderer.invoke('partial:getDiff', repoPath, filePath, target),
    applyPartialPatchTransaction: (repoPath: string, target: 'stage' | 'unstage' | 'discard' | 'staged-discard', selections: unknown[], generation: string) => ipcRenderer.invoke('partial:apply', repoPath, target, selections, generation),
    undoPartialTransaction: (transactionId: string) => ipcRenderer.invoke('partial:undo', transactionId),
    redoPartialTransaction: (transactionId: string) => ipcRenderer.invoke('partial:redo', transactionId),
