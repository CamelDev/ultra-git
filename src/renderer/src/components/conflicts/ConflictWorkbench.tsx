...
export const ConflictWorkbench: React.FC<Props> = ({ repoId, onDismiss }) => {
  ...
  useEffect(() => { if (session.activePath && session.generation && active) void loadCandidates(repoId, session.activePath) }, [repoId, session.activePath, session.generation, active?.document.path])
